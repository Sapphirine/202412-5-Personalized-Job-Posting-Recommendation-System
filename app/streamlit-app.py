...
class JobRecommender:
    ...
    def filter_jobs(self, df, location_pref: str = None, min_salary: int = None) -> pd.DataFrame:
        if location_pref:
            df = df[df['state'].str.lower().str.contains(location_pref.lower(), na=False)]
        
        if min_salary and 'salary' in df.columns:
            df = df[df['salary'] >= min_salary]
            
        return df
    
    def get_filtered_embeddings(self, df: pd.DataFrame):
        """Get embeddings only for filtered jobs"""
        filtered_indices = df.index
        
        # Select only the embeddings for filtered jobs
        filtered_bert_embeddings = self.job_embedding_bert[filtered_indices]
        filtered_tfidf_embeddings = self.job_embedding_tfidf[filtered_indices]
        
        return filtered_bert_embeddings, filtered_tfidf_embeddings
    
    
        
    def generate_recommendations(self, job_embeddings_bert, tfidf_job_descriptions, resume_embedding_bert, resume_tfidf, df, location_pref=None, min_salary=None, top_k=10):
        
        filtered_job_embeddings_bert, filtered_tfidf_job_descriptions = self.get_filtered_embeddings(
            self.filter_jobs(df, location_pref, min_salary)
        )
        cosine_scores_bert = util.cos_sim(resume_embedding_bert, filtered_job_embeddings_bert)[0].cpu().numpy()
        cosine_scores_tfidf = cosine_similarity(resume_tfidf, filtered_tfidf_job_descriptions).flatten()
...
def main():
    ...
    with st.sidebar:
        ...
        min_salary = st.number_input("Minimum Salary (USD)")
    ...
    if uploaded_file is not None:
        with st.spinner("Processing your resume..."):
            ...
            df = clean_data(df)
            filtered_df = recommender.filter_jobs(df, location_pref, min_salary)
